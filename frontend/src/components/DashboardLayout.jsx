...
import {
  ...
  User, HelpCircle, LogOut, Menu, X, Shield, Users,
} from 'lucide-react';
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-40 w-64 bg-white border-r border-gray-200 flex flex-col transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 transition-transform duration-200`}>
        ...
        {isAdmin && (
          <div className="px-3 py-2">
            <div className="flex items-center gap-2 px-3 py-1.5 bg-fct-dark text-white rounded text-xs font-semibold">
        ...
        <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
          {menuItems.map(item => (
            <NavLink key={item.to} to={item.to} end={item.to === '/admin'} onClick={() => setSidebarOpen(false)}
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2.5 rounded text-sm font-medium ${isActive ? 'bg-fct-orange text-white' : 'text-fct-dark hover:bg-gray-100'}`}
            >
        ...
        <div className="p-3 border-t border-gray-200">
          <button onClick={handleLogout} className="w-full flex items-center gap-3 px-3 py-2.5 rounded text-sm font-medium text-fct-dark hover:bg-gray-100">
